...
async def cmd_add(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_type != "private":
        await update.message.chat.send_message("🚨 安全警告：/add 功能只能在私聊中使用！")
        return

    if not context.args or "@" not in context.args[0]:
        await update.message.chat.send_message("用法：/add 账号@密码")
        return

    try:
        account, password = context.args[0].split("@", 1)
    except ValueError:
        await update.message.chat.send_message("格式错误，应为：/add 账号@密码")
        return
    ...
    if not new_cookie:
        ...
        await update.message.chat.send_message("❌ 登录失败，请检查账号密码")
        return
...
async def delete(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not args:
        return await update.message.chat.send_message("⚠️ 格式错误: /del 账号 | /del -all")

    data = load_data()
    if not is_admin(user_id):
        if args == "-all":
            if user_id not in data["users"]:
                return await update.message.chat.send_message("⚠️ 你没有绑定账号")
            deleted = list(data["users"][user_id]["accounts"].keys())
            del data["users"][user_id]
            save_data(data)
            return await update.message.chat.send_message(f"🗑 已删除所有账号: {', '.join(deleted)}")
        else:
            if user_id not in data["users"] or args not in data["users"][user_id]["accounts"]:
                return await update.message.chat.send_message("⚠️ 未找到账号")
            del data["users"][user_id]["accounts"][args]
            if not data["users"][user_id]["accounts"]:
                del data["users"][user_id]
            save_data(data)
            return await update.message.chat.send_message(f"🗑 已删除账号: {args}")
    else:
        if args.isdigit():
            if args not in data["users"]:
                return await update.message.chat.send_message("⚠️ 未找到用户")
            del data["users"][args]
            save_data(data)
            return await update.message.chat.send_message(f"✅ 已删除用户 {args} 的所有账号")
        else:
...
async def mode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args in ["true", "false"]:
        ...
        save_data(data)
        await update.message.chat.send_message(f"✅ 签到模式: {mode_text(data['users'][user_id]['mode'])}")
    else:
        await update.message.chat.send_message("⚠️ 参数错误，应为 /mode true 或 /mode false")
...
async def list_accounts(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin(user_id):
        ...
        await update.message.chat.send_message(text or "📭 暂无用户账号")
    else:
        if user_id not in data["users"] or not data["users"][user_id]["accounts"]:
            return await update.message.chat.send_message("📭 你还没有绑定账号")
        accounts = "\n".join(data["users"][user_id]["accounts"].keys())
        mode = mode_text(data["users"][user_id]["mode"])
        await update.message.chat.send_message(f"📋 你的账号:\n模式: {mode}\n{accounts}")
...
async def log(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user or not user.get("accounts"):
        return await update.message.chat.send_message("⚠️ 你还没有绑定账号，无法查询签到明细")
    ...
    if not targets[user_id]:
        if filter_acc:
            return await update.message.chat.send_message(f"⚠️ 账号 {filter_acc} 没有找到或未绑定 Cookie")
        return await update.message.chat.send_message("⚠️ 你所有账号都没有绑定 Cookie，无法查询")
    ...
    try:
        ...
        if res.returncode != 0:
            ...
            return await update.message.chat.send_message(f"⚠️ stats.js 执行失败: {res.stderr}")

        results = json.loads(res.stdout)
    except Exception as e:
        await waiting_msg.delete()
        return await update.message.chat.send_message(f"⚠️ 查询异常: {e}")
...
async def txt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = " ".join(context.args)
    if not args:
        return await update.message.chat.send_message("⚠️ 格式错误: /txt 内容 或 /txt TGID,内容")
    ...
    if "," in args and args.split(",")[0].isdigit():
        ...
        return await update.message.chat.send_message(f"✅ 已向 {target} 发送喊话")
    else:
        sent = 0
        for uid in data["users"]:
            ...
            try:
                ...
                pass
        await update.message.chat.send_message(f"✅ 已发送 {sent} 个用户")
...
async def check(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin(user_id):
        ...
        if not u or not u.get("accounts"):
            return await update.message.chat.send_message("⚠️ 你还没有绑定账号")
        targets[user_id] = u["accounts"]
        user_modes[user_id] = u.get("mode", False)

    if not targets:
        return await update.message.chat.send_message("⚠️ 没有可签到的账号")
...
async def settime(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in data.get("users", {}):
        return await update.message.chat.send_message("⚠️ 你还没有绑定账号，不能设置时间")

    if not context.args:
        return await update.message.chat.send_message("用法: /settime 小时:分钟 (0–10点)，例如: /settime 8:30")
    ...
    try:
        ...
        return await update.message.chat.send_message("⚠️ 时间格式错误，用法示例: /settime 8:30")

    # 校验范围：0–10 点
    if not (0 <= hour <= 9):
        return await update.message.chat.send_message("⚠️ 签到时间范围只能是 0–10 点")
    if not (0 <= minute < 60):
        return await update.message.chat.send_message("⚠️ 分钟必须是 0–59")
    ...
    await update.message.chat.send_message(f"✅ 已设置每日签到时间为 {hour:02d}:{minute:02d} (北京时间)")
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user = data.get("users", {}).get(user_id)
    if not user or not user.get("accounts"):
        return await update.message.chat.send_message("⚠️ 你还没有绑定账号，无法查询签到收益")
